...
for /f "delims=" %%P in ('where python 2^>nul') do (
    rem Skip MinGW/MSYS Python versions (causes compatibility issues)
    echo %%P | find /i "mingw" >nul 2>&1
    if !errorlevel! == 0 goto SKIP_PYTHON
    echo %%P | find /i "msys" >nul 2>&1
    if !errorlevel! == 0 goto SKIP_PYTHON
